...
class Home extends CI_Controller
{
	...
	private function _sendEmail($token, $type)
	{
		$config = [
			...
			'smtp_user' => 'unggarmadiii@gmail.com',
			'smtp_pass' => 'UNGGARmadi2104',
			'smtp_port' => 465,
			'mailtype' => 'html',
			'charset' => 'utf-8',
			'newline' => "\r\n"
		...
		$this->email->from('unggarmadiii@gmail.com', 'Killerbee');
	...
	public function verify()
	{
		...
		if ($user) {
			...
			if ($user_token) {
				...
				$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Your account activation failed! Wrong token!</div>');

				redirect('home');
			}
		} else {
			$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Your account activation failed! Wrong email!</div>');
	...
	public function resetPasswword()
	{
		$email = $this->input->get('email');
		$token =  $this->input->get('token');
		...
		if ($user) {
			...
			if ($user_token) {
				...
				$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Reset Pasword Failed! Wrong Token.</div>');
				redirect('auth');
			}
		} else {
